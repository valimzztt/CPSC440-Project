...
for cutoff_dict in cutoff_combinations:
    ...
    # Perform LOOCV 
    neg_mse_scores = cross_val_score(model, X, y, scoring='neg_mean_squared_error', cv=loo)
...
sc = ax.scatter(cutoff_2_body_flat, cutoff_3_body_flat, cutoff_4_body_flat, 
                c=cv_score_flat, cmap='viridis', s=40)  # Increased marker size

# Colorbar adjustments
cbar = plt.colorbar(sc, shrink=0.5, aspect=10)
cbar.set_label('CV Score (eV/atom)', fontsize=12)

# Label adjustments
ax.set_xlabel('2-body cluster cutoff (Å)', fontsize=12)
ax.set_ylabel('3-body cluster cutoff (Å)', fontsize=12)
ax.set_zlabel('4-body cluster cutoff (Å)', fontsize=12)
ax.set_title('CV Score as a Function of Cluster Diameter Cutoff (Å)', fontsize=14)

# View angle adjustment
ax.view_init(elev=30, azim=45)  # Adjust elevation and azimuth as needed

# Save the plot with higher resolution
plt.savefig(".././CPSC440-Project/figs/LOCV_Score_DiameterCutoff_Improved.png", dpi=300)
